...
from models.utils import max_connected_component


class TemporalOccGridEstimator(AbstractEstimator):
    """Temporal Occupancy grid transmittance estimator for spatial skipping.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert (
            len(roi_aabb.shape) == 2
            and roi_aabb.shape[0] == levels
            and roi_aabb.shape[1] == self.DIM * 2
        ), f"Invalid shape: {roi_aabb}!"
        ...
        self.register_buffer("aabbs", roi_aabb)  # [n_aabbs, 6]
    ...
    def sampling(
        ...
        t_idx = math.floor(t_idx * self.levels)
        intervals, samples, _ = traverse_grids(
            rays_o,
            rays_d,
            self.binaries[t_idx:t_idx+1],
            self.aabbs[t_idx:t_idx+1],
    ...
    """ Less agressive emtpy space pruning compared to the original implementation.
    """
    @torch.no_grad()
    def _update(
        self,
        ...
        occs = F.max_pool3d(
            self.occs[cell_ids].reshape(1, 1, *self.resolution),
            kernel_size=3,
            stride=1,
            padding=1,
        )[0, 0].reshape(-1)
        thre = torch.clamp(occs[occs >= 0].mean(), max=occ_thre)
        self.binaries[lvl : lvl + 1] = (occs > thre).view(
            self.binaries[lvl : lvl + 1].shape
        )

        # get maximum connected component
        mcc = max_connected_component(self.binaries[lvl : lvl + 1])
        label = torch.mode(mcc[self.binaries[lvl]], 0).values
        self.binaries[lvl : lvl + 1] = (mcc == label).view(
            self.binaries[lvl : lvl + 1].shape
        )
