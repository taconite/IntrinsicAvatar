...
class ForwardDeformer(torch.nn.Module):
    ...
    def forward(self, xd, cond, tfs, eval_mode=False):
        ...
        if eval_mode:
            ...
            fwd_tfs = torch.zeros(n_batch, n_point, n_init, 3, 3, device=xc_opt_.device)
            _, fwd_tfs[mask] = self.forward_skinning(xc_opt_, cond, tfs, mask=mask)
        ...
        if self.version == 1:
            ...
            mask = others['valid_ids']
            fwd_tfs = torch.zeros(n_batch, n_point, n_init, 3, 3, device=xc_opt.device)
            xd_opt, fwd_tfs[mask] = self.forward_skinning(xc_opt, cond, tfs, mask=mask)
