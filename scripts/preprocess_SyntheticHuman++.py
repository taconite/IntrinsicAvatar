...
if __name__ == "__main__":
    ...
    smplh2smpl = list(range(1, 22)) + [25, 40]  # SMPLH to SMPL
    ...
    for cam_idx, cam_name in enumerate(cam_names):
        ...
        for img_idx, img_file in enumerate(img_files):
            ...
            if cam_idx == 0:
                params = {
                    "poses": motion["poses"][frame_index],
                    "shapes": motion["shapes"][frame_index],
                    "Rh": motion["Rh"][frame_index],
                    "Th": motion["Th"][frame_index],
                }

                root_orient = np.array(params["Rh"]).reshape([1, 3])
                trans = np.array(params["Th"]).reshape([1, 3])

                betas = np.array(params["shapes"][None, :10], dtype=np.float32)
                poses = np.array(params["poses"][None], dtype=np.float32)
                poses_smpl = poses.reshape([-1, 3])[smplh2smpl].reshape([1, -1])
                ...
                # Get mesh vertices from EasyMocap SMPLH model
                body_model_em = SMPLlayerEM(
                    os.path.join("data/smplh", gender, "model.npz"),
                    model_type="smplh",
                    gender=gender,
                    device=device,
                    regressor_path=os.path.join(
                        "data/smplh", "J_regressor_body25_smplh.txt"
                    ),
                    mano_path="data/mano",
                    num_pca_comps=12,
                    use_pca=True,
