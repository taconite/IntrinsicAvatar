...
class NormalError(nn.Module):
    def __init__(self):
        super().__init__()

    def forward(self, inputs, targets, valid_mask, reduction='mean'):
        assert reduction in ['mean', 'none']
        inner_product = (inputs * targets).sum(dim=-1)
        a_norm = torch.linalg.norm(inputs, dim=-1)
        b_norm = torch.linalg.norm(targets, dim=-1)
        cos = inner_product / (a_norm * b_norm + 1e-8)

        # multiple with mask to take care of background
        mask = valid_mask.view(*inputs.shape[:-1])
        angle = torch.acos(cos.clamp(-1, 1)) * mask     # clamp to avoid nan
        # Note that we convert to degree before averaging
        # this could be different from certain implementations
        error = torch.rad2deg(angle)
