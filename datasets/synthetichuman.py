...
class SyntheticHumanDataset(torch.utils.data.Dataset):
    def __init__(self, data_root, subject, split, config, mode):
        ...
        self.camera_loc = self.get_camera_location(RT[:3, :3], RT[:3, 3])
        ...
        if split == "test":
            self.img_lists = sorted(glob.glob(f"{root}/images_relit/{cam_name}/*.png"))[start:end:skip]
        else:
            self.img_lists = sorted(glob.glob(f"{root}/images/{cam_name}/*.png"))[start:end:skip]
        self.albedo_lists = sorted(glob.glob(f"{root}/albedos_png/{cam_name}/*.png"))[start:end:skip]
        self.normal_lists = sorted(glob.glob(f"{root}/normals_png/{cam_name}/*.png"))[start:end:skip]
        self.msk_lists = sorted(glob.glob(f"{root}/masks/{cam_name}/*.npy"))[start:end:skip]
    ...
    def __getitem__(self, idx):
        ...
        if self.near is not None and self.far is not None:
            ...
            dist = np.sqrt(
                np.square(self.camera_loc - self.smpl_params["transl"][idx]).sum(-1)
            )
...
@datasets.register("synthetichuman")
class SyntheticHumanDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if stage in [None, "fit"]:
            ...
            self.train_dataset = SyntheticHumanDataset(
                data_root,
                self.config.subject,
                split,
        ...
        if stage in [None, "fit", "validate"]:
            ...
            self.val_dataset = SyntheticHumanDataset(
                data_root,
                self.config.subject,
                split,
                self.config.opt.get(split),
        ...
        if stage in [None, "test"]:
            ...
            self.test_dataset = SyntheticHumanDataset(
                data_root,
                self.config.subject,
                split,
                self.config.opt.get(split),
        ...
        if stage in [None, "predict"]:
            split = self.config.train_split
            self.predict_dataset = SyntheticHumanDataset(
                data_root,
                self.config.subject,
                split,
                self.config.opt.get(split),
